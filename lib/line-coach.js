...
//
// Returns the swept rows so callers can log a count. Idempotent: a
// second sweep finds nothing to update.
...
const BRAND_FIELDS = ['menu_items', 'sides', 'quality_tips', 'hold_times', 'service_hours', 'recap_recipients'];
...
// chart with the 8/10/12-min SLA reference bands. Backed by
// percentile_cont via a SQL RPC; falls back to in-app computation if
// the RPC isn't installed yet, so the chart works on day 1 of deploy.

export async function getTicketTimePercentiles({ storeId, days = 7 } = {}) {
...
export async function buildDailyRecap({ storeId, slaBreachMin = 10 } = {}) {
  if (!storeId) return { data: null, error: { message: 'storeId required' } };
  const db = getServiceClient();
  const { dayStr, startIso, endIso } = laDayBoundaries(1);
