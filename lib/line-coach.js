...
export async function upsertOrderByToastId(toastOrderId, order) {
  ...
  if (existing) {
    ...
    // created:false → this was a Toast re-delivery/update, so side
    // effects that must fire once per order (bird consumption) skip.
...
const BRAND_FIELDS = ['menu_items', 'sides', 'quality_tips', 'hold_times', 'service_hours', 'recap_recipients', 'default_languages', 'dining_option_labels', 'checklists'];
...
const HOLD_TIME_DEFAULTS = {
  ...
  // Bird oven log (lc_bird_batches): rotisserie cook time and the
  // Culinary OS locked 40-minute carve window from pull. Past the
  // window the display flags the batch SHRED (pulled/shredded use
  // only, never carve-to-order).
...
function brandDefaults() {
  return {
    ...
    // Shape: [{ id, name: {en,es}, window: {start,end}|null,
    // items: [{ id, en, es }] }]. Completion lives per store per
    // business day in lc_checklist_runs, never here.
    checklists: [],
  };
}
...
export async function registerDevice(device) {
  const db = getServiceClient();
  // Drop undefined keys so a caller can say "only touch the fields I
  // passed". PostgREST's upsert updates exactly the columns present, so
  // omitting device_name leaves the admin's name intact rather than
  // nulling it on every heartbeat.
  ...
  // Pairing and re-pairing both come through here; the auth cache must
  // not keep serving the pre-write row.
  invalidateDeviceCache(device.device_id);
  return res;
}
...
export async function deleteDevice(deviceId) {
  ...
  // Deleting the row IS a revocation — requireDevice rejects a token
  // whose device is gone. Without this the deleted device would keep
  // writing for up to the cache TTL.
  invalidateDeviceCache(deviceId);
  return res;
}

// ── Device auth ─────────────────────────────────────────
// requireDevice (lib/auth.js) calls getDeviceForAuth on every write —
// bump, unbump, heartbeat, checklist toggle, bird log. Checklist
// toggles in particular fire on every tap, so this gets the same 60s
// in-memory cache treatment as the brand config rather than a DB round
// trip per tap.
//
// The TTL is the revocation lag: after an admin revokes a device it can
// still write for up to 30s on whichever Lambda instances have it
// cached. That is an accepted trade — this is a "the iPad walked off"
// control, not a "stop an attack in progress" one — and revoking is
// idempotent, so an impatient admin clicking twice costs nothing.
// setDeviceRevoked clears the cache on the instance that served the
// request, which usually makes it feel instant in the admin.

const DEVICE_CACHE_TTL_MS = 30_000;
const _deviceCache = new Map(); // device_id → { row, expiresAt }

export function invalidateDeviceCache(deviceId) {
  if (deviceId) _deviceCache.delete(deviceId);
  else _deviceCache.clear();
}

export async function getDeviceForAuth(deviceId) {
  if (!deviceId) return null;
  const hit = _deviceCache.get(deviceId);
  if (hit && Date.now() < hit.expiresAt) return hit.row;

  const db = getServiceClient();
  const { data, error } = await withRetry(() =>
    db.from('lc_devices')
      .select('device_id, store_id, station, device_type, meta, token_issued_at, revoked_at')
      .eq('device_id', deviceId)
      .maybeSingle()
  );
  // On a DB error return null rather than caching a miss — the caller
  // treats null as "unknown device" and 401s. Better to reject a write
  // during an outage than to cache a phantom for 30s.
  if (error) return null;

  _deviceCache.set(deviceId, { row: data || null, expiresAt: Date.now() + DEVICE_CACHE_TTL_MS });
  return data || null;
}

export async function setDeviceRevoked({ deviceId, revoked, reason = null }) {
  const db = getServiceClient();
  const res = await withRetry(() =>
    db.from('lc_devices')
      .update(revoked
  ...
  try {
    ...
    db.from('lc_webhook_log')
      .insert({
        ...
        // 'hmac' | 'bearer' | 'user_agent' — which check let this request
        // through. Recorded so the unauthenticated User-Agent fallback in
        // the webhook route can be removed once we can prove nothing is
        // relying on it. Null on requests that never authenticated.
...
export function laDayBoundaries(daysAgo = 1) {
...
// ── Checklists ──────────────────────────────────────────
// Brand-wide templates (lc_brand_config.checklists) + per-store
// per-business-day completion (lc_checklist_runs). A run is keyed by
// (store_id, checklist_id, business_date) so a new LA day means a new
// key — there is no reset cron. The kiosk-facing GET merges templates
// and runs so displays never depend on the hourly config poll for
// checklist content.

// Normalize one checklist template. Ids are load-bearing (runs
// reference them) so a template without an id is dropped rather than
// silently assigned a new one.
export function normalizeChecklist(cl) {
  if (!cl || typeof cl !== 'object') return null;
  const id = typeof cl.id === 'string' && cl.id ? cl.id : null;
  if (!id) return null;
  const win = cl.window;
  const window = (win && typeof win === 'object' && win.start && win.end)
    ? { start: String(win.start), end: String(win.end) }
    : null;
  const items = Array.isArray(cl.items)
    ? cl.items
        .filter((it) => it && typeof it === 'object' && typeof it.id === 'string' && it.id)
        .map((it) => ({ id: it.id, ...normalizeTip(it) }))
        .filter((it) => (it.en && it.en.trim()) || (it.es && it.es.trim()))
    : [];
  return { id, name: normalizeTip(cl.name), window, items };
}

export function normalizeChecklists(list) {
  if (!Array.isArray(list)) return [];
  return list.map(normalizeChecklist).filter(Boolean);
}

// LA wall-clock helpers for window/business-date math. All stores are
// LA (same assumption as laDayBoundaries above).
function laDateStr(at = new Date()) {
  return new Intl.DateTimeFormat('en-CA', {
    timeZone: LA_TZ, year: 'numeric', month: '2-digit', day: '2-digit',
  }).format(at); // 'YYYY-MM-DD'
}

function laMinutesOfDay(at = new Date()) {
...
// due_now + business_date for a checklist at `at`. The one tricky case
// is an overnight window (closing 20:00–02:00): the early-morning tail
// is still the PREVIOUS business day's close — 1:30 AM closing work
// belongs to yesterday's run, and the next evening starts fresh.
// No window (or a malformed one) ⇒ "anytime" checklist: always due,
// keyed to today.
export function checklistWindowState(checklist, at = new Date()) {
  const today = laDateStr(at);
  const win = checklist?.window;
  if (!win || !win.start || !win.end) return { dueNow: true, businessDate: today };
  const [sh, sm] = String(win.start).split(':').map(Number);
  const [eh, em] = String(win.end).split(':').map(Number);
  if (![sh, sm, eh, em].every(Number.isFinite)) return { dueNow: true, businessDate: today };
  const startMin = sh * 60 + sm;
  const endMin = eh * 60 + em;
  const nowMin = laMinutesOfDay(at);
  if (endMin >= startMin) {
    return { dueNow: nowMin >= startMin && nowMin <= endMin, businessDate: today };
  }
  // Overnight wrap. At or before `end` → in-window, yesterday's date.
  if (nowMin <= endMin) {
    return { dueNow: true, businessDate: laDateStr(new Date(at.getTime() - 24 * 3600_000)) };
  }
  return { dueNow: nowMin >= startMin, businessDate: today };
}

// Kiosk/phone read: every template (with items) merged with its
// current run for the window-correct business date.
export async function getChecklistRuns(storeId, at = new Date()) {
  const brand = await getBrandConfig();
  const checklists = normalizeChecklists(brand.checklists).filter((cl) => cl.items.length > 0);
  if (checklists.length === 0) {
    return { data: { store_id: storeId, checklists: [] }, error: null };
  }
  const states = checklists.map((cl) => ({ cl, ...checklistWindowState(cl, at) }));
  const dates = [...new Set(states.map((s) => s.businessDate))];
  const db = getServiceClient();
  const { data: runs, error } = await withRetry(() =>
    db.from('lc_checklist_runs')
      .select('checklist_id, business_date, checked_items, completed_at, completed_by')
      .eq('store_id', storeId)
      .in('business_date', dates)
  );
  if (error) return { data: null, error };
  const runFor = (clId, date) =>
    (runs || []).find((r) => r.checklist_id === clId && r.business_date === date) || null;
  return {
    data: {
      store_id: storeId,
      checklists: states.map(({ cl, dueNow, businessDate }) => {
        const run = runFor(cl.id, businessDate);
        return {
          id: cl.id,
          name: cl.name,
          window: cl.window,
          ...
          due_now: dueNow,
          run: run
            ? { checked_items: run.checked_items || {}, completed_at: run.completed_at, completed_by: run.completed_by }
            : null,
        };
      }),
    },
...
// Atomic per-item toggle via the lc_toggle_checklist_item RPC (see
// scripts/add-checklists.sql). business_date is recomputed server-side
// — kiosk clocks are never trusted. Error codes callers switch on:
// 'not_found' (unknown checklist/item) and 'completed' (run already
// signed — surfaces as 409).
export async function toggleChecklistItem({ storeId, checklistId, itemId, checked, deviceId }) {
  const brand = await getBrandConfig();
  const cl = normalizeChecklists(brand.checklists).find((c) => c.id === checklistId);
  if (!cl) return { data: null, error: { code: 'not_found', message: 'Unknown checklist' } };
  if (!cl.items.some((it) => it.id === itemId)) {
    return { data: null, error: { code: 'not_found', message: 'Unknown checklist item' } };
  }
  const { businessDate } = checklistWindowState(cl);
  const db = getServiceClient();
  const { data, error } = await withRetry(() =>
    db.rpc('lc_toggle_checklist_item', {
      p_store_id: storeId,
  ...
  if (!row) return { data: null, error: { code: 'completed', message: 'Checklist already signed for this business day' } };
  return { data: row, error: null };
}

// Sign off a run. Every template item must be checked first; the
// conditional update (.is completed_at null) makes double-signing from
// two tablets a clean 409 instead of an overwrite.
export async function completeChecklistRun({ storeId, checklistId, initials }) {
  const brand = await getBrandConfig();
  const cl = normalizeChecklists(brand.checklists).find((c) => c.id === checklistId);
  if (!cl) return { data: null, error: { code: 'not_found', message: 'Unknown checklist' } };
  const { businessDate } = checklistWindowState(cl);
  const db = getServiceClient();
  const { data: run, error: readErr } = await withRetry(() =>
    db.from('lc_checklist_runs')
  ...
  if (run?.completed_at) return { data: null, error: { code: 'completed', message: 'Checklist already signed for this business day' } };
  const checkedIds = Object.keys(run?.checked_items || {});
  const missing = cl.items.filter((it) => !checkedIds.includes(it.id)).map((it) => it.id);
  if (missing.length > 0) {
    return { data: null, error: { code: 'incomplete', message: 'Unchecked items remain', missing } };
  }
  const nowIso = new Date().toISOString();
  const { data: updated, error } = await withRetry(() =>
    db.from('lc_checklist_runs')
      .update({ completed_at: nowIso, completed_by: initials, updated_at: nowIso })
      .eq('store_id', storeId)
      .eq('checklist_id', checklistId)
      .eq('business_date', businessDate)
  ...
  if (error) return { data: null, error };
  if (!updated) return { data: null, error: { code: 'completed', message: 'Checklist already signed for this business day' } };
  return { data: updated, error: null };
}

// ── Bird oven log ───────────────────────────────────────
// Batches of rotisserie birds: oven-in → pulled (warmer) → resolved
// ('used' or 'shredded'). Facts live in lc_bird_batches; the cook /
// carve-window thresholds live in hold_times so the admin can tune
// them. State is derived from timestamps client-side.

// Active batches plus anything resolved today (for the shift counter).
// Pass { daysAgo } to instead get EVERY batch put in the oven on that
// LA day (audit view — admin Shift Summary).
export async function getBirdBatches(storeId, { daysAgo = null } = {}) {
  const db = getServiceClient();
  const cols = 'id, qty, consumed_qty, in_oven_at, pulled_at, resolved_at, resolution, device_id';
  if (daysAgo != null) {
    const { startIso, endIso } = laDayBoundaries(daysAgo);
    const { data, error } = await withRetry(() =>
      db.from('lc_bird_batches')
        .select(cols)
        .eq('store_id', storeId)
        .gte('in_oven_at', startIso)
        .lt('in_oven_at', endIso)
        .order('in_oven_at', { ascending: true })
    );
    if (error) return { data: null, error };
    return { data: data || [], error: null };
  }
  const { startIso } = laDayBoundaries(0);
  const { data, error } = await withRetry(() =>
    db.from('lc_bird_batches')
      .select(cols)
      .eq('store_id', storeId)
      .or(`resolved_at.is.null,resolved_at.gte.${startIso}`)
      .order('in_oven_at', { ascending: true })
  );
  if (error) return { data: null, error };
  return { data: data || [], error: null };
...
// How many rotisserie birds one unit of a menu item draws from the
// warmer. Admin override wins (menu_items[].bird_units, Menu tab);
// the name matcher covers the obvious carve cuts. Braised proteins
// and non-chicken items are 0 — they don't pull from the carve line.
const BIRD_UNIT_DEFAULTS = [
  [/^whole bird/i, 1],
  [/^half bird/i, 0.5],
  [/^quarter bird/i, 0.25],
];

export function birdUnitsForItem(name, menuItems) {
  const lower = (name || '').toLowerCase();
  const match = (menuItems || []).find((m) => (m?.name || '').toLowerCase() === lower);
  if (match && match.bird_units != null && Number.isFinite(Number(match.bird_units))) {
    return Number(match.bird_units);
  }
  for (const [re, units] of BIRD_UNIT_DEFAULTS) {
    if (re.test(name || '')) return units;
  }
  return 0;
}

export function orderBirdUnits(items, menuItems) {
  return (items || []).reduce(
    (sum, it) => sum + (Number(it?.quantity) || 1) * birdUnitsForItem(it?.name, menuItems),
    0
  );
}

// FIFO draw-down against warmer batches (lc_consume_birds RPC — row
// locks serialize concurrent webhooks; fully consumed batches
// auto-resolve as 'used'). Fire-and-forget from the webhook: a
// consumption failure must never fail order ingest.
export async function consumeBirds({ storeId, units }) {
  if (!Number.isFinite(units) || units <= 0) return { data: 0, error: null };
  const db = getServiceClient();
  const { data, error } = await withRetry(() =>
    db.rpc('lc_consume_birds', { p_store_id: storeId, p_units: units })
  );
  if (error) return { data: null, error };
  return { data: Number(data) || 0, error: null };
}

export async function addBirdBatch({ storeId, qty, deviceId }) {
  const n = Math.floor(Number(qty));
  if (!Number.isFinite(n) || n < 1 || n > 24) {
    return { data: null, error: { code: 'bad_qty', message: 'qty must be 1-24' } };
  }
  const db = getServiceClient();
  return withRetry(() =>
    db.from('lc_bird_batches')
      .insert({ store_id: storeId, qty: n, device_id: deviceId || null })
      .select()
      .single()
  );
}

// Conditional transitions: the WHERE clauses make double-taps and
// two-tablet races resolve to a clean 'conflict' instead of clobbering
// timestamps.
export async function markBirdBatchPulled({ storeId, batchId }) {
  const db = getServiceClient();
  const nowIso = new Date().toISOString();
  const { data, error } = await withRetry(() =>
    db.from('lc_bird_batches')
      .update({ pulled_at: nowIso, updated_at: nowIso })
      .eq('id', batchId)
      .eq('store_id', storeId)
      .is('pulled_at', null)
      .select()
  ...
  if (error) return { data: null, error };
  if (!data) return { data: null, error: { code: 'conflict', message: 'Batch already pulled (or unknown)' } };
  return { data, error: null };
}

export async function resolveBirdBatch({ storeId, batchId, resolution }) {
  if (resolution !== 'used' && resolution !== 'shredded') {
    return { data: null, error: { code: 'bad_resolution', message: "resolution must be 'used' or 'shredded'" } };
  }
  const db = getServiceClient();
  const nowIso = new Date().toISOString();
  const { data, error } = await withRetry(() =>
    db.from('lc_bird_batches')
      .update({ resolved_at: nowIso, resolution, updated_at: nowIso })
      .eq('id', batchId)
      .eq('store_id', storeId)
  ...
  if (error) return { data: null, error };
  if (!data) return { data: null, error: { code: 'conflict', message: 'Batch not in warmer (or already resolved)' } };
  return { data, error: null };
}

// Mis-log escape hatch: delete an unresolved batch outright ("I fat-
// fingered +6"). Resolved rows are history and can't be deleted here.
export async function undoBirdBatch({ storeId, batchId }) {
  const db = getServiceClient();
  const { data, error } = await withRetry(() =>
    db.from('lc_bird_batches')
      .delete()
  ...
  if (error) return { data: null, error };
  if (!data) return { data: null, error: { code: 'conflict', message: 'Batch not found (or already resolved)' } };
  return { data, error: null };
}
