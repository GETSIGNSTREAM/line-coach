...
// (after dashes are normalized to '-'); never written back anywhere.
// The 'protein plate - <sku>' entries map Layer 3's four-SKU section
// titles onto the legacy per-protein Market Plate items Line Coach
// still carries — targets are the POST-alias form ('market plate' →
// 'protein plate' has already rewritten the menu-item side).
const NAME_ALIASES = {
  'arroz rojo': 'spanish rice',
  'chipotle tinga': 'chicken tinga',
  'salsa tatemada': 'charred tomato salsa',
  'market plate': 'protein plate',
  'protein plate - breast': 'boneless breast protein plate',
  'protein plate - tinga': 'chicken tinga protein plate',
  'protein plate - verde': 'pollo verde protein plate',
};

// One Notion section that specs several menu items. Verified live Jul
// 2026: "Chicken Meal — Standard Build" is the single build guide for
// the quarter/half/whole bird meals. Keys and values are canonical
// (post-alias) names.
const MULTI_TARGETS = {
  'chicken meal': ['quarter bird', 'half bird', 'whole bird'],
};
...
// Service", "Protein Plate — Universal Build Order", "Charro Beans
// Add-On"); strip the decoration before comparing. Dashes are
// normalized to '-' first so em/en-dash variants in Notion never
// break a match.
const TITLE_DECORATION_RE = /\s*(?:-\s*)?(standard build|universal build order|line reference|service|add-?on|portion|build)\s*$/i;

// Exported for tests (scripts/, node scratch checks) — not used
// outside this module at runtime.
export function canonicalName(name) {
  let n = (name || '').trim().toLowerCase().replace(/[—–]/g, '-').replace(TITLE_DECORATION_RE, '');
...
// Side sections ("Sweet Potatoes — Service", "Charro Beans Add-On")
// map to lc_brand_config.sides rows via canonicalSideName — the same
// resolver the batching math uses, so Notion titles and Toast typos
// land on one configured name. Exported for tests.
export function matchRecipeToSide(title, sides) {
  const stripped = (title || '').trim().toLowerCase().replace(/[—–]/g, '-').replace(TITLE_DECORATION_RE, '');
  const canonical = canonicalSideName(stripped);
  if (!canonical) return null;
  const lower = canonical.toLowerCase();
  return sides.find((s) => (s?.name || '').toLowerCase() === lower) || null;
}

// A recipe section can target one item (normal), several (MULTI_
// TARGETS, e.g. the Chicken Meal build → all three bird sizes), or
// none. Exported for tests.
export function matchRecipeToItems(title, menuItems) {
  const exact = menuItems.find((m) => m.name === title);
  if (exact) return [exact];
  const canon = canonicalName(title);
  const multi = MULTI_TARGETS[canon];
  if (multi) {
    const targets = menuItems.filter((m) => multi.includes(canonicalName(m.name)));
    if (targets.length) return targets;
  }
  const single = menuItems.find((m) => canonicalName(m.name) === canon);
...
export async function syncRecipesFromNotion({ dry = false } = {}) {
  ...
  for (const recipe of recipes) {
    // Entrees first (exact + canonical + multi-target), then sides via
    // canonicalSideName — powers both Learn mode and the Recipes
    // reference overlay.
    const items = matchRecipeToItems(recipe.title, menuItems);
    const side = items.length === 0 ? matchRecipeToSide(recipe.title, sides) : null;
    if (items.length === 0 && !side) {
    ...
    try {
      ...
      // Translate once per recipe — multi-target sections (Chicken
      // Meal → three bird sizes) share the same steps.
      const targetLabel = items.length > 0 ? items.map((i) => i.name).join(' / ') : side.name;
      const bilingual = await translateSteps(client, model, targetLabel, steps);
      ...
      if (items.length > 0) {
        ...
        side.build_steps = bilingual;
        side.build_steps_synced_at = syncedAt;
        matchedSides += 1;
      }
    } catch (err) {
  ...
  if (!dry && (matched > 0 || matchedSides > 0)) {
    const { error } = await updateConfig('hollywood', { menu_items: menuItems, sides });
    if (error) {
